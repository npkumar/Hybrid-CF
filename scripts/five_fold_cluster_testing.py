...
def getMAE(ranking,user,type='org'):
    ...
    for (i,v) in ranking:
        if v in removed_items:
            ...
            curdic[v] = i
      #new today for only predicted values      
...
#         org_prefs = {}
#         org_prefs = original_prefs
#         org_prefs[user] = original_prefs[user]
#         print "Length of org_prefs %d"%(len(org_prefs))
        
        dumpname = "../pickle/" +cid + "/oitemsim_" + str(b) + ".pickle"
        ...
        ranking = original.getRecommendedItems(new_prefs,itemsim,user)

        print 'Calculating MAE for original...'
        ormae = getMAE(oranking,user, 'org')
        if (ormae != None):
            original_MAE.append(ormae)

        print 'Calculating MAE for algorithm...'
        armae = getMAE(ranking,user, 'algo')
        if (armae != None):
            algorithm_MAE.append(armae)
    ...
    #pickle.dump(orgdetails, open("../rating_details/" + cid +"/orgdetails", "wb"))
    #pickle.dump(algodetails,open("../rating_details/" + cid +"/algodetails","wb"))
